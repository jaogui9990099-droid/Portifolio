...
public class PriorityThreadPoolScheduler implements JobScheduler {
    ...
    private void scheduleRetry(Job job) {
        ...
        log.info("retry scheduled for job id={} delay={}ms", job.id(), delayMs);
        ScheduledExecutorHolder.scheduler.schedule(
                () -> enqueue(job), delayMs, TimeUnit.MILLISECONDS
        );
    }

    // in production this would dispatch to a registered handler by job name.
    // for now just simulate work proportional to priority tier.
    ...
    // lazy-init avoids creating the thread before the container is ready
