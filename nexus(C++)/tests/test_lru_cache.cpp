...
TEST(LruCache, EvictsLruEntry) {
    ...
    cache.get(1); // 1 is now MRU: order = 2, 3, 1
...
TEST(LruCache, UpdateMovesToFront) {
    ...
    cache.put(1, 10);
    cache.put(2, 20);
    cache.put(1, 99); // update 1 -> moves to front
...
TEST(LruCache, TtlExpiry) {
    ...
    LruCache<std::string, int> cache(10, duration<double>(0.05)); // 50ms TTL
    cache.put("x", 42);
