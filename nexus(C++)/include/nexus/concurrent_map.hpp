...
// lock-free concurrent hash map using Robin Hood open addressing.
//
// read path (get): seqlock -- spins until it sees a consistent snapshot.
// write path (put/remove): per-slot spin lock for exclusive mutation.
//
// load factor is kept <= 0.75, table doubles and rehashes when exceeded.
// deletion uses backward-shift so we never need tombstones.
template<
    typename K,
    typename V,
...
public:
    ...
    [[nodiscard]] std::optional<V> get(const K& key) const {
        ...
        for (std::size_t probe = 0; probe <= mask_; ++probe) {
            ...
            // seqlock read -- retry if a writer is active (odd version)
            uint32_t ver;
            do { ver = slot.version.load(std::memory_order_acquire); }
            while (ver & 1u);

            if (!slot.occupied) {
                if (slot.version.load(std::memory_order_acquire) == ver)
                    return std::nullopt;
                probe = 0; continue;
            }

            // Robin Hood invariant: if this slots own displacement is shorter
            // then ours, the key cant be any further along
    ...
    bool remove(const K& key) {
        ...
        for (std::size_t probe = 0; probe <= mask_; ++probe) {
            ...
            // found -- mark empty then backward-shift to plug the gap
            // track position by index, not reference, since Slot has atomic members
    ...
    // visit every key-value pair -- dont mutate the map from inside f
    template<typename F>
...
private:
    ...
    static void insert_into(std::vector<Slot>& table, std::size_t mask, K key, V value) {
        ...
        for (std::size_t probe = 0; probe <= mask; ++probe, ++disp) {
            ...
            // Robin Hood: steal from the slot with lower displacement
