...
static void test_alignment(void) {
    ...
    /* All returned pointers must be aligned to max_align_t. */
    for (int i = 1; i < 64; ++i) {
...
static void test_reset_reuses_memory(void) {
    ...
    void* p2 = forge_arena_alloc(&a, 256);
    ASSERT_NOTNULL(p2);
    /* After reset the bump pointer is back to zero; first alloc from same base. */
    ASSERT_EQ(p1, p2);
...
static void test_overflow_chaining(void) {
    ...
    forge_arena_t a;
    forge_arena_init(&a, 128); /* deliberately small */

    /* Allocate more than the primary node can hold. */
    int success = 1;
    for (int i = 0; i < 32; ++i) {
        void* p = forge_arena_alloc(&a, 64);
