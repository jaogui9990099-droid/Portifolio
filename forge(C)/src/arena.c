...
void* forge_arena_alloc_aligned(forge_arena_t* arena, size_t size, size_t align) {
    ...
    // current node is full, chain an overflow arena
    // size the overflow at least as big as the current block so we
    // dont end up with a chain of tiny arenas for large allocations
    if (!arena->next) {
...
void forge_arena_reset(forge_arena_t* arena) {
    ...
    // free overflow chain but keep the primary buffer alive so the
    // backing pages stay warm and we dont thrash malloc
