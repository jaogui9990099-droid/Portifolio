...
public record JobResponse(
    ...
    public record ExecutionSummary(
        ...
        public static ExecutionSummary from(ExecutionResult r) {
            return new ExecutionSummary(r.id(), r.attempt(), r.success(),
    ...
    public static JobResponse from(Job job) {
        return new JobResponse(
                job.id(), job.name(), job.payload(), job.status(), job.priority(),
                job.maxRetries(), job.attemptCount(), job.timeoutSeconds(),
                job.submittedAt(), job.lastUpdatedAt(),
