...
class JobServiceTest {
    ...
    void submit_persistsJobAndEnqueues() {
        Job result = service.submit(Job.builder().name("test-job").payload("{}").build());
    ...
    void submit_defaultsToNormalPriority() {
        Job result = service.submit(Job.builder().name("job").build());
    ...
    void cancel_succeedsForPendingJob() {
        ...
        assertThat(service.cancel(job.id())).isTrue();
        assertThat(repository.findById(job.id()).map(Job::status)).contains(JobStatus.CANCELLED);
    }
    ...
    void list_filtersByStatus() {
        service.submit(Job.builder().name("a").build());
        service.submit(Job.builder().name("b").build());
        var pending = service.list(JobStatus.PENDING, null, 0, 10);
        assertThat(pending).hasSize(2).allMatch(j -> j.status() == JobStatus.PENDING);
    }
